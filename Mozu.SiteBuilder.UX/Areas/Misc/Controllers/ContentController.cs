...
namespace Mozu.SiteBuilder.UX.Areas.Misc.Controllers
{
    ...
    public class ContentController : ApiControllerBase
    {
        ...
        public ContentController(ContentFetcher docRepo, IApiContext appCtx, ISettings settings, Lazy<IGeneralSettingsWebApiClient> generalSettingsWebApiClient)
        {
            ...
            int? quality = null)
        {
         ...
         //  {
           ...
           /*}).CloneWithConfigOptions(x =>
           {
               //copy over RP version Header if Present
            ...
            if (th.IfModifiedSince.HasValue || shouldRedirectToCdn)
            {
                ...
                {
                    result = await _docRepo.GetTreeDocumentContentHead(headers,list, documentId).ConfigureAwait(false);
                }
                ...
                if (shouldRedirectToCdn && !isRewrite && result.Content.Headers.LastModified.HasValue) return RedirectToCdn(list, documentId, result.Content.Headers.LastModified.Value);
                if (th.IfModifiedSince.GetValueOrDefault(DateTime.MinValue) >= result.Content.Headers.LastModified.GetValueOrDefault(DateTimeOffset.MaxValue)) return StatusCode(304);
            }
    ...
    public class ContentFetcher : ServiceClientMessageHandler
    {
        ...
        public ContentFetcher(HttpClient httpClient, 
            ...
            IEnumerable<IOutboundRequestHandler> outputRequestHandlers = null) : base(context, settings, cacheFactory, outputRequestHandlers)
        {
        ...
        {
            var relpath =  Format(documentListName,false)+"/documents/"+Format(documentId,false)+"/transform?width="+Format(width,true)+"&height="+Format(height,true)+"&maxWidth="+Format(maxWidth,true)+"&maxHeight="+Format(maxHeight,true)+"&crop="+Format(crop,true)+"&quality="+Format(quality,true);
            var requestInfo = GenerateRequestUrlInfo(relpath, ServiceId, this.ApiContext?.MozuInstanceId, new ConfigOptions());
        ...
        public virtual Task<HttpResponseMessage> TransformTreeDocumentContent(
            ...
            CancellationToken cancellationToken = default) {
            var relpath =  Format(documentListName,false)+"/documentTree/"+Format(documentName,false)+"/transform?width="+Format(width,true)+"&height="+Format(height,true)+"&maxWidth="+Format(maxWidth,true)+"&maxHeight="+Format(maxHeight,true)+"&crop="+Format(crop,true)+"&quality="+Format(quality,true)+"&responseFields="+Format(responseFields,true);
            var requestInfo = GenerateRequestUrlInfo(relpath, ServiceId, this.ApiContext?.MozuInstanceId, new ConfigOptions());
            var request = new HttpRequestMessage
            {
                Method = new HttpMethod("GET"),
        ...
        public virtual Task<HttpResponseMessage> GetDocumentContentHead(
            Dictionary<string,string> headers,
            string documentListName, string documentId, string includeInactive = null, string responseFields = null, TargetContextLevelType targetContextLevel = TargetContextLevelType.NotSpecified,
            CancellationToken cancellationToken = default)
        {
            var relpath =  Format(documentListName,false)+"/documents/"+Format(documentId,false)+"/content?includeInactive="+Format(includeInactive,true)+"&responseFields="+Format(responseFields,true);
            var requestInfo = GenerateRequestUrlInfo(relpath, ServiceId, this.ApiContext?.MozuInstanceId, new ConfigOptions());
            var request = new HttpRequestMessage
            {
                Method = new HttpMethod("GET"),
        ...
        public virtual Task<HttpResponseMessage> GetTreeDocumentContentHead(  Dictionary<string,string> headers,string documentListName, string documentName, string responseFields = null, TargetContextLevelType targetContextLevel = TargetContextLevelType.NotSpecified,CancellationToken cancellationToken = default) {
            var relpath =  Format(documentListName,false)+"/documentTree/"+Format(documentName,false)+"/content?responseFields="+Format(responseFields,true);
            var requestInfo = GenerateRequestUrlInfo(relpath, ServiceId, this.ApiContext?.MozuInstanceId, new ConfigOptions());
            var request = new HttpRequestMessage
            {
                Method = new HttpMethod("GET"),
