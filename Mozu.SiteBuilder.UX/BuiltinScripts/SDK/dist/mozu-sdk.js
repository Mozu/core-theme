        ...
        exportFn(function() {
            ...
            var IframeXHR = function(window, document, undefined) {
                var hasPostMessage = window.postMessage && navigator.userAgent.indexOf("Opera") === -1, firefoxVersion = function() {
                    var ua = navigator.userAgent, re = /Firefox\/(\d+)/i, match = ua.match(re), versionStr = parseInt(match ? match[1] || false : false), version = isNaN(versionStr) ? false : versionStr;
                    return version;
                }(), cacheBust = 1, hashRE = /^#?\d+&/, originRE = /^https?:\/\/[^/]+/i, validateOrigin = function(ixhr, origin) {
                    return ixhr.frameOrigin === origin.toLowerCase().match(originRE)[0];
                }, messageDelimiter = "|||||", messageMethods = hasPostMessage ? {
                    listen: function() {
                        var self = this;
                ...
                var pub = {
                    ...
                    getActionsFor: function(typeName) {
                        ...
                        for (var a in basicOps) {
                            if (isSimpleType || !(a in objectTypes[typeName])) actions.push(a);
                        }
                        if (!isSimpleType) {
            ...
            var ApiObject = function() {
                ...
                ApiObjectConstructor.getHydratedType = function(typeName) {
                    if (!(typeName in this.hydratedTypes)) {
                        var availableActions = ApiReference.getActionsFor(typeName), reflectedMethods = {};
                    ...
                    action: function(instanceOrType, actionName, data) {
                        ...
                        return me.request(ApiReference.basicOps[actionName], requestConf, data).then(function(rawJSON) {
                            if (requestConf.returnType) {
                                ...
                                if (rawJSON || rawJSON === 0 || rawJSON === false) obj.data = utils.clone(rawJSON);
                                delete obj.unsynced;
