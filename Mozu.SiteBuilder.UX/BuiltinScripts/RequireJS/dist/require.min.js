 ...
 */
define(["text"],function(n){var r="Unable to parse shim dependency.",t={},e=/[^\[>]+/,o=/(.+)=([a-zA-Z_$][0-9a-zA-Z_$]*)$/,s=/>([^\]]+)$/,u=function(n){var a=n.indexOf("["),v=n.lastIndexOf("]"),y=e.exec(n),p=s.exec(n),w=[],b=[],k=[],d,g,t;if(!y)throw r;if(d=y[0],g=p?p[1]:null,a!==-1&&v!==-1){var i=n.substring(a+1,v),c,u,f=0,nt=-1,l=!1,h;for(t=0;t<i.length;t++){if(h=i.charAt(t),h==="["&&f++,h==="]"&&f--,l=h===",",f<0)throw r;(l||t+1===i.length)&&f===0&&(c=i.substring(nt+1,l?t:t+1),u=o.exec(c),u?(w.push(u[1]),k.push(u[2])):b.push(c),nt=t)}}return{name:d,deps:w.concat(b),args:k,toExport:g}},f="define('{4}',[{0}], function({1}) { {2} ; return {3} });\n\n\n//@ sourceURL={4}.js\n\n",h=f.replace("'{4}',",""),i=function(n,t,i){var r=n.deps.length>0?"'"+n.deps.join("','")+"'":"";return(i?f:h).replace("{0}",r).replace("{1}",n.args.join(",")).replace("{3}",n.toExport).split("{4}").join(n.name).replace("{2}",t)};return{write:function(r,f,e){var o=u(f);t.hasOwnProperty(o.name)?e.asModule(r+"!"+f,i(o,t[o.name])):n.get(require.toUrl(o.name),function(n){t[o.name]=n;e.asModule(r+"!"+f,i(o,n))})},load:function(r,f,e,o){var s=u(r),h=function(n){o.isBuild&&(t[s.name]=n);eval(i(s,n,!0));f([s.name],e)};h.error=function(){e.error.apply(e,arguments)};n.load(s.name+".js",f,h,o)}}})