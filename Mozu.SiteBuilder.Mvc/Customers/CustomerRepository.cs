...
namespace Mozu.SiteBuilder.Mvc.Customers
{
    public class CustomerRepository : ICustomerRepository
    {
        ...
        {
            var result = await _customerAccountWebApiClient.GetCustomerAccounts(startIndex, pageSize, sortyBy, responseGroups, filter).Result.ReadAsAsync();
        ...
        {
            var result = await _customerAccountWebApiClient.GetCustomerAccount(customerId).Result.ReadAsAsync();
        ...
        public async Task<CustomerAccount> Update(SB.CustomerAccount customerAccount, int? customerId)
        {
            ...
            var result = await _customerAccountWebApiClient.UpdateCustomerAccount(account, customerId).Result.ReadAsAsync();
        ...
        public async Task<StreamContent> Delete(SB.CustomerAccount customerAccount)
        {
            ...
            return await _customerAccountWebApiClient.DeleteCustomerAccount(customerAccount.Id).Result.ReadAsAsync();
        }

        public async Task<List<CustomerAccountNote>> GetCustomerNotes(int? customerAccountId, int? startIndex = 0, int? pageSize = 25)
        {
            var task = await _customerAccountWebApiClient.GetCustomerAccountNotes(customerAccountId, startIndex, pageSize, null, null);
        ...
        public async Task<CustomerAccountNote> CreateCustomerNote(SB.CustomerAccountNote customerAccountNote, int? customerAccountId)
        {
            ...
            var task = await _customerAccountWebApiClient.AddCustomerAccountNote(note, customerAccountId);
            var result = task.ReadAsAsync().Result;
