...
namespace Mozu.SiteBuilder.Mvc.Settings
{
    ...
    public class SettingsRepository : ModelBase, ISettingsRepository
    {
        ...
        private Task<UX.Models.Settings.GeneralSettings> _generalSettingsTask;

        public SettingsRepository(IGeneralSettingsWebApiClient generalSettingsWebApiClient, ISiteBuilderApiContext siteBuilderApiContext)
        {
        ...
        public Task<UX.Models.Settings.GeneralSettings> GetGeneralSettings()
        {
            if (_generalSettingsTask != null)
            {
                return _generalSettingsTask;
            }

            var key = string.Format("{0}{1}{2}", _siteBuilderApiContext.SiteId, _siteBuilderApiContext.TenantId, typeof (GeneralSettings).FullName);

            var settings = System.Web.HttpRuntime.Cache.Get(key) as UX.Models.Settings.GeneralSettings;
            if (settings == null)
            {
                return _generalSettingsTask = _generalSettingsWebApiClient.GetGeneralSettings().ContinueWith(x =>
                    {
                        var res = x.Result;
                        try
                        {
            ...
            else
            {
                ...
                return _generalSettingsTask = tcs.Task;
            }
        }

        private Task<SettingsContainer> _settingsTask;

        public Task<SettingsContainer> GetSettings()
        {
            if (_settingsTask != null)
            {
                return _settingsTask;
            }
            else
            {
                var genSettingsTask = this.GetGeneralSettings();
                if (genSettingsTask.IsCompleted)
                {
                    var tcs = new TaskCompletionSource<SettingsContainer>();
                    tcs.SetResult(new SettingsContainer()
                                      {
                                          General = genSettingsTask.Result
                                      });
                    return _settingsTask = tcs.Task;
                }
                else
                {
                    return _settingsTask = genSettingsTask.ContinueWith(x => new SettingsContainer()
                                      {
                                          General = x.Result 
                                      });
                }
    ...
    //public class SettingsRepository2 :  IAlternateNamingValueContainer
    //{
    ...
    //    public SettingsRepository2(IProvisioningWebApiClient provClient, ICheckoutSettingsWebApiClient checkoutSettingsWebApiClient, IShippingSettingsWebApiClient shippingSettingsWebApiClient)
    //    {
