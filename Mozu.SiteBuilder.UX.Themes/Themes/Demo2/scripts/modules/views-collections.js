...
define(['modules/jquery-mozu', 'underscore', 'hyprlive', 'modules/backbone-mozu', "modules/models-faceting", "modules/views-productlists", "modules/views-paging"], function($, _, Hypr, Backbone, FacetingModels, ProductListViews, PagingViews) {
    ...
    function factory(conf) {
        ...
        router.origionalPathname = location.pathname;
        model.on('facetchange', function(q) {
            if (!navigating) {
                var testUrl = router.origionalPathname + q;
                $.ajax(
                    {
                        url: testUrl,
                        method: 'HEAD',
                        ...
                        complete: function (xhr) {
                           // router.navigate(q);
                            if (xhr.status == 200 && xhr.getResponseHeader('Location')&& window.history)
                            {
                                window.history.pushState({}, document.title, xhr.getResponseHeader('Location'));
                            } else {
                                router.navigate(q);
                            }
                        }
