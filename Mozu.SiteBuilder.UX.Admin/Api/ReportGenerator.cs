...
namespace Mozu.SiteBuilder.UX.Admin.Api
{
    ...
    public class ReportGenerator : IReportGenerator
    {
        ...
        public async Task<string> GenerateDashboardUri(ReportParams reportParams)
        {
            ...
            if ( string.IsNullOrEmpty(orgId) || string.IsNullOrEmpty(secret) || string.IsNullOrEmpty(url))
            ...
            if (reportParams.SiteId != 0)
                env.Add("SITEID", reportParams.SiteId);
            ...
            request.Add("dashboard", reportParams.ChartioId);
            request.Add("env", env);
            var token = JWT.JsonWebToken.Encode(request, secret, JWT.JwtHashAlgorithm.HS256);

            return string.Format("{0}/d/{1}/{2}", url, reportParams.ChartioId, token);
        }
