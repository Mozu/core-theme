...
{ 
    //ALL MAGIC STRINGS... 
      [ServiceContract]
    public class NavigationOldController : BaseController
    {
        ...
        private  async  Task<bool> ProcessRoot(System.Collections.Generic.List<NavigationTreeNode> resItems, UX.Models.Navigation.NavigationSet navSet)
        {
            ...
            IEnumerable<NavigationTreeNode> newUnAssignedPages = pages.Items.Where(x => !navSet.Nodes.Any(_ => _.OriginalId == x.Id || _.OriginalId == x.Name)).Select(x => x.Map<NavigationTreeNode>());
            ...
            if (blogs.Count > 0)
            {
                if ( !navSet.Nodes.Any ( _ => _.OriginalId == blogs[0].Id ))
                {
