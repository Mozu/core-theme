...
{
    [WebApi("app/accountattributes", SuppressDescriptorGeneration = true)]
    public class AccountAttributeController : BaseController
    {
        private readonly IAccountAttributeDefinitionWebApiClient _accountAttributeDefinitionWebApiClient;

        public AccountAttributeController(
            IAccountAttributeDefinitionWebApiClient accountAttributeDefinitionWebApiClient)
        {
            _accountAttributeDefinitionWebApiClient =
                accountAttributeDefinitionWebApiClient.CloneWithApiContext(x => x.LocaleCode = "en-US");
        }
            ...
            [FromUri] FilterCollection extFilter)
        {
            ...
            {
                var dcItem = (await _accountAttributeDefinitionWebApiClient.GetAttribute(pagingParams.id))
                    .ReadAsSync();
                var vmItem = Mapper.Map<AttributeModel>(dcItem);
                return List2(vmItem);
            }
            var results = (await _accountAttributeDefinitionWebApiClient.GetAttributes(
        ...
        public async Task<Response<List<AttributeModel>>> CreateAttribute([FromBody] List<AttributeModel> attributes)
        {
            var tasks = attributes.Select(Mapper.Map<AttributeDC>)
                .Select(a => _accountAttributeDefinitionWebApiClient.CreateAttribute(a))
                .ToList();
        ...
        public async Task<Response<List<AttributeModel>>> EditAttribute(List<AttributeModel> attributes)
        {
            var tasks = attributes.Select(Mapper.Map<AttributeDC>).Select(a =>
            {
                a.AttributeCode = string.IsNullOrEmpty(a.AttributeCode)
                    ...
                    : a.AttributeCode;
                return _accountAttributeDefinitionWebApiClient.UpdateAttribute(a.AttributeFQN, a);
            }
            ).ToList();
            await Task.WhenAll(tasks);
        ...
        {
            var tasks = attributes.Select(Mapper.Map<AttributeDC>)
                .Select(a => _accountAttributeDefinitionWebApiClient.DeleteAttribute(a.AttributeFQN))
                .ToList();
            await Task.WhenAll(tasks);
