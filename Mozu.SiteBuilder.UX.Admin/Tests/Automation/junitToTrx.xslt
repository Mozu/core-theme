...
<xsl:transform version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:variable name="guidStub">
    <xsl:call-template name="testRunGuid">
      <xsl:with-param name="date" select="/testsuite/@timestamp"/>
      <xsl:with-param name="time" select="/testsuite/@time"/>
    </xsl:call-template>
  </xsl:variable>
  ...
  <xsl:template match="/">
    <TestRun xmlns="http://microsoft.com/schemas/VisualStudio/TeamTest/2010">
      ...
      <xsl:variable name="pass_count" select="count(//testcase[not(failure)])"/>
      <xsl:variable name="failed_count" select="count(//failure)"/>
      <xsl:variable name="inconclusive_count" select="count(//testcase[@success='False' and (count(./failure) + count(./reason) = 0)])"/>
      <xsl:variable name="executed_count" select="count(//testcase[@executed='True'])"/>
      <xsl:variable name="notExecuted_count" select="count(//testcase[@executed='False'])"/>
      <ResultSummary>
