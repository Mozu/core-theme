    ...
    filterTreeBy : function (params, scope) {
        ...
        var collectNodes    = function (node) {
            ...
            if (node.data.leaf) {
                ...
                // include all parent nodes in result set originally, except the root one 
                if (rootVisible || node != root) linearNodes[ linearNodes.length ] = node;
                
                var nodeMatches;
                
                // mark matching nodes to be 
                if (checkParents) {
        ...
        // removes the parent nodes which do not match themselves and have no macthing children  
        var nodesToKeep = [];
