...
import {
    isInEditableDropzone
} from './../utilities/factory';
...
export default class Editor {
    ...
    updateAllContentWidgets() {
        Array.from(document.querySelectorAll(BLOCK_SELECTOR)).forEach((block) => {
    ...
    setDirtyState(val) {
        contentWidget.toggleAllContentWidgets();
        contentWidget.hideEditor();
    ...
    showLayoutHeaders(bool) {
        Array.from(document.querySelectorAll(LAYOUT_WIDGET_HEADER_SELECTOR)).forEach((el) => {
            el.style.display = bool ? 'block' : 'none';
        });

        Array.from(document.querySelectorAll('.mz-layout-widget, .mz-cms-col-')).forEach((el) => {
            if (bool && isInEditableDropzone(el)) {
        ...
        Array.from(document.querySelectorAll(BLOCK_SELECTOR)).forEach((block) => {
            if (bool && isInEditableDropzone(block)) {
    ...
    showDropZones() {
        Array.from(document.querySelectorAll('.mz-cms-grid, .mz-cms-col-')).forEach((grid) => {
            grid.classList.add(MZ_CMS_SHOW_CLASS);
        });
    }

    hideDropZones() {
        Array.from(document.querySelectorAll('.mz-cms-grid, .mz-cms-col-')).forEach((grid) => {
            grid.classList.remove(MZ_CMS_SHOW_CLASS);
        });
    }
    ...
    persistanceData() {
        ...
        Array.from(document.querySelectorAll('.mz-cms-grid')).forEach((grid) => {
