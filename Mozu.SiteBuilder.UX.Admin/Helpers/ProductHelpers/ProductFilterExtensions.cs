    ...
    internal static class ProductFilterExtensions
    {
        ...
        private static string GetFilter(object value , FilterCollectionItem filter)
        {
            switch (filter.property.ToLowerInvariant())
            {
                ...
                case "modifiedfrom" :
                return string.Format("updatedate gt {0}", ((DateTime)filter.value).ToUniversalTime().ToString("o"));

                case "modifiedto":
                return string.Format("updatedate lt {0}", ((DateTime)filter.value).ToUniversalTime().ToString("o"));
                case "price":
